...
public:
	...
	*/
	...
	/**
	*	@brief	Tree의 node를 스크린에 출력한다.(변수가 포인터 형식인 경우)
	*	@pre	none
	*	@post	스크린에 Tree가 InOrder, PreOrder, PostOrder 방법으로 각각 출력됨.
	*/
	//void pointprint();
...
private:
	...
	*	트리 전부 출력
	*/
	void PrintInOrderTraversal(TreeNode<T>* root);
	/**
	*	트리 전부 출력(데이터가 포인터인 경우)
	*/
	//void PointPrintInOrderTraversal(TreeNode<T>* root);
...
AVLTree2<T>::~AVLTree2()
{
	MakeEmpty(root); // 파라미터에 루트를 주어서 트리를 다 삭제함
}
template<typename T>
int AVLTree2<T>::GetLength()
{
	return this->length;
}
template<typename T>
bool AVLTree2<T>::IsEmpty()
{
	if (length == 0)
		return true;
	else
...
template<typename T>
void AVLTree2<T>::MakeEmpty(TreeNode<T>*& Node)
{
	if (Node == nullptr) // 비어있다면
		return;
	else
	{
		MakeEmpty(Node->left); // 왼쪽으로 재귀함수실행
		MakeEmpty(Node->right); // 오른쪽으로 재귀함수실행
		delete Node; // 노드를 지워줌 재귀함수이므로 리프노드부터 지워진다.
		Node = nullptr; // 포인터의 래퍼런스를 눌로 만듬
	}
}
template<typename T>
int AVLTree2<T>::Add(T data)
{
...
template<typename T>
T AVLTree2<T>::Get(T data)//여기서 T는 포인터 형 변수임을 잊지 말것.
{
	TreeNode<T>* temp;// 임시변수
	temp = root;
	while (temp != nullptr)
	...
	return nullptr; // 못찾았을경우 펄스
}

template<typename T>
T AVLTree2<T>::GetByPoint(T data)
{
	TreeNode<T>* temp;// 임시변수
	temp = root;
	while (temp != nullptr)
...
template<typename T>
void AVLTree2<T>::Replace(T data)
{
	TreeNode<T>* temp;// 임시변수
	temp = root;
...
/*
template<typename T>
void AVLTree<T>::pointprint()
{
	//ostream &out;
	PointPrintInOrderTraversal(root);			// InOrder 방법으로 출력
}
*/
// Tree의 node를 각각의 방법대로 출력함
template<class T>
void AVLTree2<T>::SaveTree(ofstream& fout)
{
	SaveInOrderTraversal(root, fout);			// InOrder 방법으로 출력
}

template<typename T>
int AVLTree2<T>::Max(int i, int j)
{
	return i > j ? i : j;
}
template<typename T>
int AVLTree2<T>::getHeight(TreeNode<T>* Node)
{
...
template<typename T>
int AVLTree2<T>::getBalance(TreeNode<T>* Node)
{
	return getHeight(Node->left) - getHeight(Node->right); // 왼쪽 높이 - 오른쪽높이
}
template<typename T>
TreeNode<T>* AVLTree2<T>::Rotate_LL(TreeNode<T>* parent)
{
...
template<typename T>
TreeNode<T>* AVLTree2<T>::Rotate_RR(TreeNode<T>* parent)
{
...
template<typename T>
TreeNode<T>* AVLTree2<T>::Rotate_LR(TreeNode<T>* parent)
{
...
template<typename T>
TreeNode<T>* AVLTree2<T>::Rotate_RL(TreeNode<T>* parent)
{
...
template<typename T>
TreeNode<T>* AVLTree2<T>::MakeBalance(TreeNode<T>* Node)
{
...
template<typename T>
TreeNode<T>* AVLTree2<T>::RecursiveInsert(TreeNode<T>* Node, T data)
{
...
template<typename T>
TreeNode<T>* AVLTree2<T>::Recursivedelete(TreeNode<T>* Node, T data)
{
...
template<typename T>
void AVLTree2<T>::SaveInOrderTraversal(TreeNode<T>* root, ofstream& out)
{
	MasterType temp;
	if (root != NULL)								// root가 존재하는 경우
	{
