...
*/
	...
	template<typename T>
	void PushAllFolders(Stack<T>* temp){
		PushFolder(root,temp);
	}
};
...
int insert(node<T>*&root, T Item) {
	if (root == nullptr) {
		...
		if (root->info->GetExt()==Item->GetExt()) {//확장자까지 같은 때
			cout << "\tSame Item Already Exist!\n";
			return 0;
		}
		else if (root->info->GetExt() < Item->GetExt()) {
			insert(root->right, Item);
		}
		else if (root->info->GetExt() > Item->GetExt()) {
			insert(root->left, Item);
...
void search(const node<T>* root, T& item, bool& found) {
	if (root->info->GetName() == item->GetName()) {
		found = true;
		item = root->info;
	}
	else if (root->info->GetName() < item->GetName() &&root->right != nullptr) {
		search(root->right, item, found);
	}
	else if (root->info->GetName() > item->GetName() &&root->left != nullptr) {
		search(root->right, item, found);
	}
}
...
void PushFolder(node<T> *root,  Stack<T1>* temp) {
	if (root->info->GetExt() == ".fol") {
		temp->push(&root->info);
	}
	if (root->left != nullptr) {
		PushFolder(root->left,temp);
	}
	if (root->right != nullptr) {
		PushFolder(root->left,temp);
	}
