...
public:
	...
	*/
	FolderType()
	{
		...
		m_list = new  UnsortedList<AllType*>;
	}
	/**
	*	destructor.
	*/
...
*/
	virtual AllType* SearchFolder(AllType *temp) ;
...
*/
	virtual AllType* SearchFile(AllType *temp);
	/**
...
*	@return	왼쪽 name변수가 오른쪽 name 변수보다 크면 true,아니면 false
	...
	*/
	virtual UnsortedList<AllType*> Getsublist() {
		return *m_list;
	}
private:
	int memory;
	...
	int subfoldernum;
	int filenum;
	//oublySortedLinkedList<FolderType> *fdFolderList;
	//DoublySortedLinkedList<FileType> *fdFileList;
	UnsortedList<AllType*> *m_list;
	int size;
};
